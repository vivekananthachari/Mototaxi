...
public class ChartActivity extends Activity
{
	...
	public void onCreate(Bundle savedInstanceState)
	{
		...
		setTheme(MainActivitynew.nightMode ? R.style.AppTheme_Dark : R.style.AppTheme);
		getWindow().setFlags(WindowManager.LayoutParams.FLAG_FULLSCREEN,
		                     WindowManager.LayoutParams.FLAG_FULLSCREEN);

		// keep main display on?
		if(MainActivitynew.prefs.getBoolean("keep_screen_on", false))
		{
		...
		MainActivitynew.setFixedPids(pidNumbers);
		// if auto hiding selected ...
		if(MainActivitynew.prefs.getBoolean(MainActivitynew.PREF_AUTOHIDE,false))
		{
			// get autohide timeout [s]
			int timeout = Integer.parseInt(
				MainActivitynew.prefs.getString(MainActivitynew.PREF_AUTOHIDE_DELAY,"15") );
			// auto hide toolbar
	...
	private transient final Handler mHandler = new Handler()
	{
		...
		public void handleMessage(Message msg)
		{
			...
			switch (msg.what)
			{
				case MainActivitynew.MESSAGE_UPDATE_VIEW:
					/* update chart */
					chartView.invalidate();
					break;

				// set toolbar visibility
				case MainActivitynew.MESSAGE_TOOLBAR_VISIBLE:
	...
	private final TimerTask updateTask = new TimerTask()
	{
		...
		public void run()
		{
			...
			Message msg = mHandler.obtainMessage(MainActivitynew.MESSAGE_UPDATE_VIEW);
